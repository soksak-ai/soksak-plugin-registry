...
[ -f "$root/pnpm-lock.yaml" ] || { echo 'BUILD_DECLARATION_INVALID: pnpm-lock.yaml is required' >&2; exit 78; }
...
lock=$(shasum -a 256 "$root/pnpm-lock.yaml" 2>/dev/null | awk '{print $1}')
printf 'BUILD_ENVIRONMENT_READY node=%s pnpm=%s runtime=%s/%s lockSHA256=%s\n' "$node_actual" "$pnpm_actual" "$node_platform" "$node_arch" "${lock:-missing}"
